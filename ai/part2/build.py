...
def get_html_page(url,filename,tableid,flag,winner_flag,matchno):
    	f = u.urlopen(url)
	html = f.read()
	...
	if winner_flag == 0:
		...
		win = w.split('>')
#		print win[1]
		winner = win[1].split('<');d = str(winner).split(' won');v = str(matchno)+str(',')+str(d[0].replace("['",""));
		fp2 = open("wonby.txt","a")
		fp2.write(v)
		fp2.write("\n")
		fp2.close()
...
#						print "found"
						...
						c = b[1].replace("\n","");d = c.split(' (');qwe = str(matchno)+str(',')+str(d[0])+str(',')+str(d[1].replace(')',''));
						fp2 = open("mom.txt","a")
						fp2.write(qwe)
						fp2.write("\n")
						fp2.close()
...
#						print "found"
						b = str(list1[li+1]).split()
						toss = str(matchno)+str(',')+str(b[1][:-1])
                        fp2 = open("wontoss.txt","a");
                        fp2.write(toss);
                        fp2.write("\n");
                        fp2.close();
...
#	os.chdir("./201225020_assignment2/")
	...
	get_html_page(url1,filename11,table1,3,0,1)
	get_html_page(url1,filename12,table2,0,1,1)
	get_html_page(url1,filename13,table3,3,1,1)
	get_html_page(url1,filename14,table4,0,1,1)

	os.chdir("../")
	os.mkdir("match2")
	os.chdir("./match2/")

	get_html_page(url2,filename21,table1,3,0,2)
	get_html_page(url2,filename22,table2,0,1,2)
	get_html_page(url2,filename23,table3,3,1,2)
	get_html_page(url2,filename24,table4,0,1,2)
	os.chdir("../")
	os.mkdir("match3")
	os.chdir("./match3/")

	get_html_page(url3,filename31,table1,3,0,3)
	get_html_page(url3,filename32,table2,0,1,3)
	get_html_page(url3,filename33,table3,3,1,3)
	get_html_page(url3,filename34,table4,0,1,3)

	os.chdir("../")
	os.mkdir("./match4")
	os.chdir("./match4/")
	get_html_page(url4,filename41,table1,3,0,4)
	get_html_page(url4,filename42,table2,0,1,4)
	get_html_page(url4,filename43,table3,3,1,4)
	get_html_page(url4,filename44,table4,0,1,4)

	os.chdir("../")
	os.mkdir("./match5")
	os.chdir("./match5/")
	get_html_page(url5,filename51,table1,3,0,5)
	get_html_page(url5,filename52,table2,0,1,5)
	get_html_page(url5,filename53,table3,3,1,5)
	get_html_page(url5,filename54,table4,0,1,5)
	os.chdir("../")
	os.chdir("../")
